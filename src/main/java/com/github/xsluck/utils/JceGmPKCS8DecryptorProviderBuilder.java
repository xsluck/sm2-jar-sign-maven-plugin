...
public class JceGmPKCS8DecryptorProviderBuilder {
    private JcaJceHelper helper;

    public JceGmPKCS8DecryptorProviderBuilder() {
        helper = new DefaultJcaJceHelper();
    }

    public JceGmPKCS8DecryptorProviderBuilder setProvider(String providerName) {
        helper = new NamedJcaJceHelper(providerName);
        return this;
    }

    public JceGmPKCS8DecryptorProviderBuilder setProvider(Provider provider) {
            ...
            public InputDecryptor get(final AlgorithmIdentifier algorithm)
                ...
                try {
                    if (PEMUtilities.isPKCS5Scheme2(algorithm.getAlgorithm())) {
                        ...
                        if (oid.equals(GMObjectIdentifiers.sms4_cbc.getId())) {
                            algParams = helper.createAlgorithmParameters("SM4");
                        } else {
                            algParams = helper.createAlgorithmParameters(oid);
                        }
